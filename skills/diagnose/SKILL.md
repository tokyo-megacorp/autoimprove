...
- **`skills/_shared/evaluate.sh`** — The evaluator uses the same extraction patterns that diagnose probes. If `diagnose` shows a metric as `[OK]`, evaluate.sh will extract it correctly. If `[BROKEN]`, fix it before running the loop.
