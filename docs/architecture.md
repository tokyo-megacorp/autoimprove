...
The adversarial review is a separate feature from the grind loop. It runs a structured debate to find bugs in code.

```
Orchestrator
  ├── Round 1
  │     ├── Enthusiast (finds issues) → JSON findings
  │     ├── Adversary (challenges findings) → JSON verdicts
  │     └── Judge (arbitrates) → JSON rulings + convergence flag
  ├── Round 2 (if not converged)
  │     └── ... (prior round context fed to each agent)
  └── Convergence check (deterministic, orchestrator-side)
```

**Three agents, sequential per round:**
- **Enthusiast** — maximum recall, flag everything suspicious (scored by severity)
- **Adversary** — maximum precision, debunk false positives (3x penalty for wrong debunks)
- **Judge** — impartial arbiter with symmetric scoring (+5/-5 for correct/incorrect rulings)

**Convergence:** The orchestrator compares `(finding_id, winner, final_severity)` tuples between consecutive rounds. When the sets are identical, the debate has stabilized. The Judge's self-reported `convergence` flag is supplemental — never the stop signal.

**Telemetry:** Each run is stored at `~/.autoimprove/runs/<RUN_ID>/` with incremental per-round files and a final summary. See the [output schema reference](../skills/adversarial-review/references/output-schema.md).

---

## Idea Matrix Architecture

The idea matrix systematically explores design options using a 3x3 grid of parallel haiku agents.

```
Orchestrator (opus/sonnet)
  ├── Research codebase → architecture brief (~500 tokens)
  ├── Generate 9 cell assignments
  ├── Pack 9 agent prompts (~800 tokens each, self-contained)
  └── Dispatch 9 idea-explorers (haiku, no tools, parallel)
        ├── Cell 1: Option A alone
        ├── Cell 2: Option B alone
        ├── Cell 3: Option C alone
        ├── Cell 4: A + B hybrid
        ├── Cell 5: A + C hybrid
        ├── Cell 6: B + C hybrid
        ├── Cell 7: A + B + C trio
        ├── Cell 8: Best-of-breed remix (or Alt 1)
        └── Cell 9: Contrarian approach (or Alt 2)
```

**Key design choice:** The orchestrator does all codebase research. Haiku agents receive pre-digested context and reason about it — they never touch the codebase. This makes them faster, cheaper, and more focused.

**Scoring rubric:** Each cell is scored on 4 dimensions (feasibility, risk, synergy potential, implementation cost), all 1-5, directionally consistent (higher = better, risk inverted).

**Convergence synthesis:** The orchestrator ranks cells by composite score, filters dealbreakers, analyzes score band distribution and cluster patterns, then recommends a design with conditions and top insights from across the matrix.
